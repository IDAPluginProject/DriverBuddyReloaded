...
### Function-name sets
All heuristic, callchain, and scoring function-name lists live in `signatures.py` (the single source of truth, not scattered across modules), imported by convention as `from DriverBuddyReloaded import signatures as sig`: `DANGEROUS_SINKS`, `VALIDATION_FUNCS`, `PRIVILEGE_GATE_FUNCS`, `PRIVILEGED_SENSITIVE_OPS`, `IRQL_RAISING_FUNCS`, `MDL_USER_FUNCS`, `COPY_SINKS`, `ALLOCA_FUNCS`, `POOL_ALLOC_FUNCS`, `PROBE_FUNCS`, `FREE_POOL_FUNCS`. Opcode/instruction severities live in `OPCODE_SEVERITY` (named-opcode scan) and `PRIV_INSN_SEVERITY` (in/out/`mov cr*`/etc. flagged by `heuristics.check_privileged_instructions`). Add to the relevant set in `signatures.py` when expanding coverage; modules import the set by name. The two audit checks have their own name sets in `signatures.py`: `DEVICE_CREATE_UNSECURED_FUNCS` (ACL audit, `utils.find_device_create_calls`) and `SYMLINK_CREATE_FUNCS` (symlink-exposure audit, `device_name_finder.find_symbolic_links`); both resolve names via `ctx.functions_map` (superset of `imports_map`) so WDF named functions are covered alongside ntoskrnl imports. The severity constants (`SEV_*`), feature flags, and tuning constants those sets reference remain in `config.py`.
